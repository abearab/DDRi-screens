...
def writeToCounts(fileTup):
    ...
    print('Percent A sgRNA and B sgRNA mismatch',
          statsCounts['A sgRNA and B sgRNA do not match'] * 100.0 / statsCounts['All sgRNAs uniquely map'])
    print('Percent both A and B match',
          statsCounts['A sgRNA and B sgRNA match'] * 100.0 / statsCounts['All sgRNAs uniquely map'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('Seq_File_Names', nargs='+',
                        help='Name(s) of sequencing file(s). '
                             'Unix wildcards can be used to select multiple files at once. '
                             'The script will search for all *.fastq.gz, *.fastq, and '
    ...
    guideTable = pd.read_csv(args.Guide_Table, sep=',', header=0)[
        ['sgID_AB', 'sgID_A', 'protospacer_A', 'sgID_B', 'protospacer_B']]
    ...
    guideTable['protospacer_A'] = guideTable['protospacer_A'].str[:19].str.upper()
